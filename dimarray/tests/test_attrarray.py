...
class test_AttrArray(TestCase):
    ...
    def test_nanstd(self):
        ...
        for d in range(3):
            ...
            assert_array_almost_equal(
                dat.std(0, ddof=d), dat.nanstd(0, ddof=d))
            self.assertEqual(dat.nanstd(0, ddof=d).name, 'randvals')
            assert_array_almost_equal(
                dat.std(1, ddof=d), dat.nanstd(1, ddof=d))
            self.assertEqual(dat.nanstd(1, ddof=d).name, 'randvals')
            assert_array_almost_equal(
                dat.std(2, ddof=d), dat.nanstd(2, ddof=d))
        ...
        # Test ddof:
        for d in range(3):
            self.assertEqual(dat[~np.isnan(dat)].std(
                ddof=d), dat.nanstd(ddof=d))
    ...
    def test_nanvar(self):
        ...
        for d in range(3):
            ...
            assert_array_almost_equal(
                dat.var(0, ddof=d), dat.nanvar(0, ddof=d))
            self.assertEqual(dat.nanvar(0, ddof=d).name, 'randvals')
            assert_array_almost_equal(
                dat.var(1, ddof=d), dat.nanvar(1, ddof=d))
            self.assertEqual(dat.nanvar(1, ddof=d).name, 'randvals')
            assert_array_almost_equal(
                dat.var(2, ddof=d), dat.nanvar(2, ddof=d))
        ...
        # Test ddof:
        for d in range(3):
            self.assertEqual(dat[~np.isnan(dat)].var(
                ddof=d), dat.nanvar(ddof=d))
