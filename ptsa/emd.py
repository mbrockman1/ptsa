...
def _localmax(d):
    ...
    maxRuns = (diffScore[realRunStarts] == -
               1) & (diffScore[realRunStops] == -1)

    # If a run is a local max, then count the middle position (rounded) as the 'max'
    # CHECK THIS
    maxRunMiddles = np.round(
        realRunStarts[maxRuns] + realRunLengths[maxRuns] / 2.) - 1
...
def calc_inst_info(modes, samplerate):
    ...
    for m in range(len(modes)):
        ...
        phase[m, :] = np.angle(h);
        f[m, :] = np.r_[np.nan,
                        0.5 * (np.angle(-h[2:] * np.conj(h[0:-2])
                                        ) + np.pi) / (2 * np.pi) * samplerate,
                        np.nan]
