...
def filtfilt2(b, a, x, axis=-1):
    ...
    bRange = list(range(edge, 1, -1))
    sBeg = 2 * x.take([0], axis).repeat(len(bRange),
                                        axis) - x.take(bRange, axis)
    eRange = list(range(-1, -edge, -1))
    sEnd = 2 * x.take([-1], axis).repeat(len(eRange),
                                         axis) - x.take(eRange, axis)
    ...
    (y, zf) = lfilter(b, a, y.take(
        list(range(y.shape[axis] - 1, -1, -1)), axis), axis, ziEnd)

    # flip it back
    y = y.take(list(range(y.shape[axis] - 1, -1, -1)), axis)
    return y.take(list(range(edge - 1, y.shape[axis] - edge + 1)), axis)
...
def firls(N, f, D=None):
    ...
    for i, (f0, f1) in enumerate(f):
        R += np.pi * f1 * sinc(2 * (j - k) * f1) - np.pi * f0 * sinc(2 * (j - k) * f0) + \
            np.pi * f1 * sinc(2 * (j + k) * f1) - np.pi * \
            f0 * sinc(2 * (j + k) * f0)

        r += D[i] * (2 * np.pi * f1 * sinc(2 * j * f1) -
                     2 * np.pi * f0 * sinc(2 * j * f0))
