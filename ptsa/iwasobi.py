...
class IWASOBI():
    ...
    def corr_est(self, x, T, q):
        ...
        for index in range(q + 1):
            ...
            R_est[:, i:i + NumOfSources] = (1 / np.float(T)) * \
                (np.dot(x[:, :T], x[:, index:T + index].T))
    ...
    def CRLB4(self, ARC):
        ...
        ISR = sum_Rs_s.T / denom * \
            np.outer(np.ones((K, 1)), Rs[0, :]) / \
            np.outer(Rs[0, :].T, np.ones((1, K)))
        # ISR(eye(K)==1)=0;
        ISR[np.eye(K) == 1] = 0
        # end %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% of CRLB4
        return ISR
    ...
    def uwajd(self, M, maxnumiter=20, W_est0=None):
        ...
        for k in range(L):
            ...
            Ms[:, ini:ini +
                d] = np.dot(np.dot(W_est, M[:, ini:ini + d]), W_est.T)
