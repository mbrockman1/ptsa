...
class EegTimeSeries_old(object):
    ...
    def __init__(self, data, samplerate, tdim=-1, offsetMS=None, offset=None, bufferMS=None, buffer=None):
        ...
        if not offset is None:
            ...
            self.offset = int(
                N.round(float(offsetMS) * self.samplerate / 1000.))
        else:
        ...
        if not buffer is None:
            ...
            self.buffer = int(
                N.round(float(bufferMS) * self.samplerate / 1000.))
        else:
        ...
        self.durationMS = (self.shape[self.tdim] -
                           2 * self.buffer) * 1000. / self.samplerate
    ...
    def resample(self, resampledRate, window=None):
        ...
        # resample the data
        newLength = N.fix(
            self.data.shape[self.tdim] * resampledRate / float(self.samplerate))
        self.data = resample(self.data, newLength,
                             axis=self.tdim, window=window)

        # set the new offset and buffer lengths
        self.buffer = int(N.round(float(self.buffer) *
                                  resampledRate / float(self.samplerate)))
        self.offset = int(N.round(float(self.offset) *
                                  resampledRate / float(self.samplerate)))
    ...
    def decimate(self, resampledRate, order=None, ftype='iir'):
        ...
        # do the decimation
        self.data = filter.decimate(
            self.data, downfact, n=order, ftype=ftype, axis=self.tdim)
