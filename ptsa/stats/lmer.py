...
def sparse_stable_svd(R, nboot=50):
    ...
    for i in range(len(boots)):
        Ub, sb, Vhb = np.linalg.svd(
            np.concatenate(R[boots[i]]), full_matrices=False)
...
def _eval_model(model_id, perm=None, boot=None):
    ...
    for i in range(len(Vh)):
        ...
        if perm is None and boot is None and mm._null_formula_str:
            ...
            for bi in range(mm._num_null_boot):
                ...
                ll_null.append(
                    float(r['logLik'](r['refit'](m_null, Dw_sim))[0]))
                ll_full.append(
                    float(r['logLik'](r['refit'](m_full, Dw_sim))[0]))
            # append to full list
    ...
    if perm is None and boot is None and mm._null_formula_str:
        ...
        print("Like Ratio Comb: ", np.dot(
            2 * (log_likes - log_likes_null), ss[ss > 0.0] / _ss))
        ...
        sscale = ss / ss.sum()
        mg = np.dot([((lr_boot[:, ii] - lr[ii]) > 0).sum()
                     for ii in range(20)], sscale)
