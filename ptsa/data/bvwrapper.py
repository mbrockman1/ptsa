...
class BVWrapper(BaseWrapper):
    ...
    def __init__(self, filepath):
        ...
        self._data_file = os.path.join(
            self.filedir, cp.get('Common Infos', 'datafile'))
        self._samplerate = float(
            10e5) / int(cp.get('Common Infos', 'samplinginterval'))
        self._markerfile = os.path.join(
            self.filedir, cp.get('Common Infos', 'markerfile'))
