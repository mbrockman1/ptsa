...
class DataWrapper(object):
    ...
    def get_event_data(self, channels, eventOffsets,
        ...
        offset_samp = int(
            np.ceil((np.abs(offset) - samplesize * .5) / samplesize) * np.sign(offset))

        # finally get the duration necessary to cover the desired span
        dur_samp = int(
            np.ceil((dur + offset - samplesize * .5) / samplesize)) - offset_samp + 1
        ...
        eventdata = self._load_data(
            channel, eventOffsets, dur_samp, offset_samp)
        ...
        if not(filtFreq is None):
            ...
            eventdata = eventdata.filter(
                filtFreq, filtType=filtType, order=filtOrder)
